...
if(dataContent.position===""){
  await listFetcher.addList({name : dataContent.name})
    ...
    const realPosition = positionNumber - 1
      await listFetcher.addList({name : dataContent.name, position:realPosition }) 
...
const main = document.querySelector("#lists-container")
 Sortable.create(main, {
  onEnd:function(evt){
    ...
    console.log(evt.clone.dataset.id)
...
const positionElem = modaleElem.querySelector('#modify-list-position')
...
function idGetter(){
 ...
 const id=parentElm.dataset.id 
  const modfyFormElem = document.querySelector('#form-list-updater')
...
function idGetter(){
 ...
 function getId(event){
  ...
  const parentElm  = btnElem.closest('.list_template-item')
 const id=parentElm.dataset.id 
  const deleteFormElem = document.querySelector('#form-list-deleter')
